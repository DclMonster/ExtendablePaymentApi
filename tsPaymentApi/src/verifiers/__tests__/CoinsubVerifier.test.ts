...
describe('CoinsubVerifier', () => {
    let verifier: CoinsubVerifier;
    const mockSecret = 'test-secret';

    beforeEach(() => {
        process.env.COINSUB_SECRET = mockSecret;
        verifier = new CoinsubVerifier();
    });

    afterEach(() => {
        jest.resetAllMocks();
        delete process.env.COINSUB_SECRET;
    });

    describe('constructor', () => {
        it('should throw error if COINSUB_SECRET is not set', () => {
            delete process.env.COINSUB_SECRET;
            expect(() => new CoinsubVerifier()).toThrow('COINSUB_SECRET not set in environment variables.');
        });
    });
    ...
    describe('getSignatureFromHeader', () => {
        it('should extract signature from headers', () => {
            const headers = { 'coinsub-signature': 'valid-signature' };
        ...
        it('should throw error for missing signature', () => {
            const headers = {};
            expect(() => verifier['getSignatureFromHeader'](headers)).toThrow('Invalid or missing CoinSub signature in headers');
        });

        it('should throw error for array signature', () => {
            const headers = { 'coinsub-signature': ['sig1', 'sig2'] };
            expect(() => verifier['getSignatureFromHeader'](headers)).toThrow('Invalid or missing CoinSub signature in headers');
        });
    });
    ...
    describe('verifyHeaderSignature', () => {
        const mockData = { test: 'data' };
        const mockHeaders = { 'coinsub-signature': 'valid-signature' };
        const mockComputedSignature = 'computed-signature';
